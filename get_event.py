...
def get_date_last_update(file_path):
...
def get_mtgtop8_format():
    ...
    return followed_format

def get_list_of_event(format_url):
    page = urlopen(format_url)
...
def get_event_data(list_of_events, date_last_update):
    base_url = "https://www.mtgtop8.com/"
    events_inforamtion = []
...
def data_to_json(dict, file_path):
    if not dict['name']: 
